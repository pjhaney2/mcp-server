...
def search_state_fips(keyword: List[str], max_results: Optional[int] = 20) -> List[Tuple[str, str]]:
    """
    Search for US states by keyword(s) and return their FIPS codes.
    ...
    Args:
        keyword (Union[str, List[str]]): Single search term or list of search terms to match against state names (case-insensitive)
    ...
    Examples:
        ...
        >>> search_state_fips(["california", "texas"])
    ...
    for kw in keywords:
        ...
        search_terms = re.findall(r'\b\w+\b', kw.lower())
        if not search_terms:
            continue
        ...
        for result in priority_1_results + priority_2_results + priority_3_results:
