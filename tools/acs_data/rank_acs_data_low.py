...
def rank_acs_data_low(
    ...
    """
    Finds the geographic areas with the lowest values (places, counties, etc.) based on a specific ACS data point.
    
    This function queries the US Census Bureau's American Community Survey (ACS) 5-year estimates
    to find the lowest values for a specified data point across all geographic areas of a given type.
    ...
    Args:
        ...
        limit (Optional[int]): Number of lowest results to return. Defaults to 20.

    Returns:
        Dict[str, Any]: Response containing:
            - status: "success" or "error"
            - data: List of areas with lowest values (if successful)
    ...
    try:
        ...
        # Sort by value in ascending order (lowest first) and limit results
        processed_data.sort(key=lambda x: x["value"], reverse=False)
        lowest_results = processed_data[:limit if limit else 20]
        
        return {
            "status": "success", 
            "data": lowest_results,
            "total_found": len(processed_data),
            "returned_count": len(lowest_results),
            "query_info": {
                "data_point": data_point,
                "geo_type": geo_type,
...
if __name__ == "__main__":
    ...
    print("Bottom 5 places in Illinois by population:")
    print(rank_acs_data_low("DP05_0001E", "place", "17", "2022", 5))
    print("\nBottom 5 counties in Illinois by population:")
    print(rank_acs_data_low("DP05_0001E", "county", "17", "2022", 5))
    
    # Test with nationwide queries (no state specified)
    print("\nBottom 5 places nationwide by population:")
    print(rank_acs_data_low("DP05_0001E", "place", None, "2022", 5))
    print("\nBottom 5 counties nationwide by population:")
    print(rank_acs_data_low("DP05_0001E", "county", year="2022", limit=5))
    print("\nBottom 5 states by population:")
    print(rank_acs_data_low("DP05_0001E", "state", limit=5))
    print("\nBottom 5 MSAs by population:")
    print(rank_acs_data_low("DP05_0001E", "metropolitan statistical area/micropolitan statistical area", limit=5))